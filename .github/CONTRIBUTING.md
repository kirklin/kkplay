...
1. `git clone https://github.com/kirklin/kkplay.git` to clone the repo.
2. `cd kkplay` to enter the project directory.
...
2. The source entry point is `src/kkplay.ts`, editing any files referenced will automatically trigger rebuild. **Make sure not to break any existing examples**.
...
Most kkplay docs are written in `src/types.ts` as [jsDoc](https://www.typescriptlang.org/docs/handbook/jsdoc-supported-types.html) above each kkplay component entry.
