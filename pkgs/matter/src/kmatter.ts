...
export default (k: KKPlayCtx): MatterPlugin => {
    ...
    // cache the last positions and angles of all objects so we know what objects transforms are changed on kkplay side
    ...
    k.onUpdate(() => {
        ...
        objs.forEach((obj: GameObj<MatterAreaComp | PosComp | RotateComp>) => {
            const id = obj.id;
            if (
                !obj.body || positions[id] === undefined
                || angles[id] === undefined
            ) {
                return;
            }
        ...
        // sync kkplay object transform with matter object transform
        objs.forEach(
            (
                obj: GameObj<
                    | MatterAreaComp
                ...
                if (obj.c("mbody")) {
                    // syncing matter.js body transform to kkplay object transform
    ...
    function mbody(opt: MatterBodyOpt = {}): MatterBodyComp {
        return {
            ...
            jump(
                this: GameObj<MatterAreaComp | MatterBodyComp>,
                force: number = 6,
            ) {
