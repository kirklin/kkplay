...
const srcDir = "src";
const distDir = "dist";
const srcPath = `${srcDir}/kkplay.ts`;
...
const fmts = [
    ...
    { format: "esm", outfile: `${distDir}/kkplay.mjs` },
];
...
export async function genDTS() {
    ...
    // generate global decls for KKPlayCtx members
    let globalDts = "";

    globalDts += "import { KKPlayCtx } from \"./types\"\n";
    globalDts += "declare global {\n";

    for (const stmt of stmts) {
        if (stmt.name === "KKPlayCtx") {
            if (stmt.kind !== "InterfaceDeclaration") {
                throw new Error("KKPlayCtx must be an interface.");
            }
            for (const name in stmt.members) {
                globalDts += `\tconst ${name}: KKPlayCtx["${name}"]\n`;
            }
    ...
    if (!globalGenerated) {
        throw new Error("KKPlayCtx not found, failed to generate global defs.");
    }
...
export function serve(opt = {}) {
    ...
    * {
    ...
    <body>
    <script src="/dist/kkplay.js"></script>
