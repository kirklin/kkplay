...
// TODO: A better way to detect KKPlayCtx
export const isKKPlayCtx = (obj: any): obj is KKPlayCtx => {
    return obj && obj["loadAseprite"];
};

export const getKKPlayContext = (fallBack?: any): KKPlayCtx => {
    if (!ctx) {
        throw new Error(
            "You are trying to access to KKPlay Context before their initialization.",
        );
    }

    if (isKKPlayCtx(fallBack)) {
        return fallBack;
    }
...
/**
 * Initialize KKPLAY context. The starting point of all KKPLAY games.
 *
 * @example
 * ```js
 * // Start KKPLAY with default options (will create a fullscreen canvas under <body>)
 * kkplay()
 *
 ...
 * })
 *
 * // All KKPLAY functions are imported to global after calling kkplay()
 * add()
 * onUpdate()
 * onKeyPress()
 * vec2()
 *
 * // If you want to prevent KKPLAY from importing all functions to global and use a context handle for all KKPLAY functions
 * const k = kkplay({ global: false })
 *
 * k.add(...)
 * k.onUpdate(...)
 * k.onKeyPress(...)
...
>(
    gopt: KKPlayOpt<TPlugins, TButtons> = {},
): TPlugins extends [undefined] ? KKPlayCtx<TButtons, TButtonsName>
    : KKPlayCtx<TButtons, TButtonsName> & MergePlugins<TPlugins> =>
{
    const root = gopt.root ?? document.body;

    // if root is not defined (which falls back to <body>) we assume user is using kkplay on a clean page, and modify <body> to better fit a full screen canvas
    ...
    function addKKPlay(p: Vec2, opt: BoomOpt = {}): GameObj {
        const kkplay = add([
            pos(p),
            stay(),
        ]);
    ...
    return ctx as TPlugins extends [undefined]
        ? KKPlayCtx<TButtons, TButtonsName>
        : KKPlayCtx<TButtons, TButtonsName> & MergePlugins<TPlugins>;
};

export default kkplay;
