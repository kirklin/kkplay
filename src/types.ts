...
/**
 * Context handle that contains every kkplay function.
 *
 * @group Start
 */
export interface KKPlayCtx<
...
> {
     ...
     * // every time an obj with tag "bomb" hits the floor, destroy it and addKKPlay()
     * on("ground", "bomb", (bomb) => {
     *     destroy(bomb)
     *     addKKPlay(bomb.pos)
     * })
     ...
     *     // predefined functions to get the default value by KKPLAY
     *     return def_vert();
     * }`,
     ...
     * @group Plugins
     */
    plug<T extends Record<string, any>>(plugin: KKPlayPlugin<T>): KKPlayCtx & T;
    /**
     ...
     * @group Misc
     */
    addKKPlay(pos: Vec2, opt?: BoomOpt): GameObj;
    /**
     * All chars in ASCII.
     *
     * @group Misc
    ...
    BLACK: Color;
    /**
     * The canvas DOM KKPLAY is currently using.
     *
     * @group Misc
     */
    ...
    /**
     * Current KKPLAY library version.
     *
     * @since v3000.0
     * @group Info
     */
...
export type PluginList<T> = Array<T | KKPlayPlugin<any>>;
...
/**
 * KKPLAY configurations.
 *
 * @group Start
 */
export interface KKPlayOpt<
...
> {
    ...
    /**
     * If KKPLAY should render a default loading screen when assets are not fully ready (default true).
     *
     * @since v3000.0
     */
    ...
    focus?: boolean;
    /**
     * If import all KKPLAY functions to global (default true).
     */
    global?: boolean;
    /**
 ...
 * @example
 * ```js
 * // a plugin that adds a new function to KKPLAY
 * const myPlugin = (k) => ({
 *    myFunc: () => {
 ...
 * @group Plugins
 */
export type KKPlayPlugin<T> = (
    k: KKPlayCtx,
) => T | ((...args: any) => (k: KKPlayCtx) => T);
...
export interface GameObjRaw {
    ...
    onKeyDown: KKPlayCtx["onKeyDown"];
    onKeyPress: KKPlayCtx["onKeyPress"];
    onKeyPressRepeat: KKPlayCtx["onKeyPressRepeat"];
    onKeyRelease: KKPlayCtx["onKeyRelease"];
    onCharInput: KKPlayCtx["onCharInput"];
    onMouseDown: KKPlayCtx["onMouseDown"];
    onMousePress: KKPlayCtx["onMousePress"];
    onMouseRelease: KKPlayCtx["onMouseRelease"];
    onMouseMove: KKPlayCtx["onMouseMove"];
    onTouchStart: KKPlayCtx["onTouchStart"];
    onTouchMove: KKPlayCtx["onTouchMove"];
    onTouchEnd: KKPlayCtx["onTouchEnd"];
    onScroll: KKPlayCtx["onScroll"];
    onGamepadButtonDown: KKPlayCtx["onGamepadButtonDown"];
    onGamepadButtonPress: KKPlayCtx["onGamepadButtonPress"];
    onGamepadButtonRelease: KKPlayCtx["onGamepadButtonRelease"];
    onGamepadStick: KKPlayCtx["onGamepadStick"];
    onButtonDown: KKPlayCtx["onButtonDown"];
    onButtonPress: KKPlayCtx["onButtonPress"];
    onButtonRelease: KKPlayCtx["onButtonRelease"];
}

/**
 * The basic unit of object in KKPLAY. The player, a butterfly, a tree, or even a piece of text.
 *
 * @group Game Obj
 */
